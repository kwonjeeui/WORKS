        ...
        $window.off('load.loadAnc').on('load.loadAnc', function () {
            try {
                ...
                setTimeout(() => {
                    ...
                    if(param == 'SM-X910NZEAKOO') {
                        $(target).find('[data-opt-key="optCdA"]').find('input[value="optA_1"]').trigger('click');
                        $(target).find('[data-opt-key="optCdB"]').find('input[value="optB_1"]').trigger('click');
                        $(target).find('[data-opt-key="optCdC"]').find('input[value="optC_2"]').trigger('click');
                        $(target).find('[data-opt-key="optCdD"]').find('input[value="optD_3"]').trigger('click');
                    }
