...
export const modalOrigin = {
    /** 모달오픈버튼 클릭시 dimm 처리와 모달 오픈 함수 */
    init() {
        
        //모달 오픈
        PT_STATE.$PROJECT.off('.click.clickModalOrigin').on('click.clickModalOrigin', '[data-role="btnModalOrigin"]', function (e) {
            e.preventDefault();
            const data = {
                $prevfocus: [],
                opt: {
            ...
            const target = $this.attr('data-target');
            const $closeModal = $(`[data-role="closeModalOrigin"][data-target="${target}"]`);
            ...
            // 모달 닫기
            PT_STATE.$PROJECT.off('click.closeModal').on('click.closeModal', '[data-role="closeModalOrigin"]', function (e) {
                e.preventDefault();
                const $this = $(this);
                const $target = $($this.attr('data-target'));
