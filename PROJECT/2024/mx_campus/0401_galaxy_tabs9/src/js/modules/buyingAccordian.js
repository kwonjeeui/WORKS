...
export const buyingAccordian = {
    ...
    toggle(params) {
        ...
        //토글 열기
        PT_STATE.$PROJECT.off('click.toggleBuyingAcc').on('click.toggleBuyingAcc', '[data-role-buying-accordian]', function (e) {
            e.preventDefault();
            const $this = $(this);
            const { target, speed = data.opt.speed, openFocus = data.opt.openFocus, offScroll = data.opt.offScroll, callback = data.opt.callback, group } = _.findItem(data.params, 'el', `[data-role-buying-accordian="${$this.attr('data-role-buying-accordian')}"]`);
            const $target = $(target);
            ...
            $this.hasClass('active') ? $target.stop().slideDown(speed, () => {$target.removeAttr("style")}) : $target.stop().slideUp(speed, () => {$target.removeAttr("style"); $target.hide()});
        ...
        //토글 닫기
        // PT_STATE.$PROJECT.off('click.closeAcc').on('click.closeAcc', '[data-role-buying-accordianClose]', function (e) {
        //     e.preventDefault();
        //     const $this = $(this);
