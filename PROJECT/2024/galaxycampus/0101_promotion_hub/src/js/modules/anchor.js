...
export const anchor = {
    ...
    load(params) {
        ...
        $window.off('load.loadAnc').on('load.loadAnc', function () {
            try {
                const { target, speed = data.opt.speed, scroll = data.opt.scroll, click = data.opt.click} = _.findItem(data.params, 'url', param);                

                console.log(_.findItem(data.params, 'url', param))
