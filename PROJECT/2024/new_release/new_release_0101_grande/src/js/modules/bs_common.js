...
export const anchor = {
    ...
    click(params) {
        ...
        PT_STATE.$PROJECT.off('click.clickAnc').on('click.clickAnc', '[data-role-anchor]', function (e) {
            ...
            if(target == '#component_tap'){
                setTimeout(function() {
                    $(`.pt_comp_youmake`).trigger('click');
                }, 100);
            }
        });
    },
    ...
    load(params) {
        ...
        $window.off('load.loadAnc').on('load.loadAnc', function () {
            try {
                ...
                if(param == 'POD_buying_Allinone') {
                    $(target).find('label[for="optA_2"]').trigger('click');
                }
                if(param == 'POD_buying_WF21D17CWWC') {
                    $(target).find('label[for="optB_3"]').trigger('click');
                }
                if(param == 'POD_buying_WF21D17CVVC') {
                    $(target).find('label[for="optB_1"]').trigger('click');
                }
                $(target).focus()
            } catch (err) {
...
export const accordian = {
    ...
    toggle(params) {
        ...
        PT_STATE.$PROJECT.off('click.toggleAcc').on('click.toggleAcc', '[data-role-accordian]', function (e) {
            ...
            // 토글클릭시 앵커이동 안되게
            if(!util.isMobile() && target == "#pt_step_cnt1" || !util.isMobile() && target == "#pt_step_cnt2" || !util.isMobile() && target == "#pt_step_cnt3" || util.isMobile() && target == "#spec_acco"){
                ...
                $('html, body').stop().animate({ scrollTop: $(target).is(':visible') && openFocus ? $(target).offset().top - navHeight : $this.offset().top - navHeight }, 500);
            }
...
export const modal = {
    ...
    init() {
        ...
        PT_STATE.$PROJECT.off('.click.clickModal').on('click.clickModal', '[data-role="btnModal"]', function (e) {
            ...
            const target = $this.attr('data-target');
            const videoSize = $this.attr('data-size');
            ...
            let navHeight = $('.sec_nav').outerHeight() * 1.2;

            data.$prevfocus.push($this);

            if(videoSize === 'mo'){
                $(target).addClass('pt_mosize');
                $target.css('top', winTop - navHeight);
            }else{
                $(target).removeClass('pt_mosize');
                $target.css('top', winTop - headerHeight - tabHeight + (winHeight - targetHeight) / 2 );
            }
            // winHeight > targetHeight ? $target.css('top', winTop - headerHeight - tabHeight + (winHeight - targetHeight) / 2 ) : $target.css('top', winTop);
            // $target.css('top', winTop + 50)
