...
function setOption(buying, dataList, groupList){
    dataList.forEach(function(item){
        ...
        groupList[item.group].forEach(function(groupItem){
            if(!!groupItem.optCdColor && groupItem.optCdColor != '-') {
                arrColor.push(groupItem.optCdColor+'|'+groupItem.optNmColor+'|'+groupItem.optColor);
            }
        ...
        let nextItem = [];
        let arrOptions = ['optCdColor', 'optCdA', 'optCdB'];
        arrOptions.forEach(function(opts,arrOptIdx){
            Object.keys(item).forEach(key => {
                ...
                if(opts === key && optVal != '-' && optVal.length > 0){
                    if(nextOpt){
                        nextItem = groupList[item.group].filter((gItem) => gItem[opts] == item[opts]);
...
export class Buying {
    ...
    renderOption(_key,_checked,item) {
        ...
        if(buying.params.type === 'multi' || buying.params.type === 'list'){
            ...
            const state = allList.filter( (item) => item.default === 'O')[0].state;
    ...
    changeOptEtc(item) {
        ...
        if(item){
            const group = $(item).attr('data-buying-group');
            const allList = buying.params.parsedData.listGroup[group];
            state = allList.filter( (item) => item.default === 'O')[0].state;
        }
    ...
    getListPaging(listDefault){
        ...
        if(!listDefault){
            const list = params.pdList;
            const visibleData = list.filter((item) => item.hidden.trim().toUpperCase() != 'O');
            listDefault = !!visibleData[0].default ? visibleData.filter((item) => item.default.trim().toUpperCase() == 'O') : [];
        }
    ...
    clickBtnCate() {
        ...
        $btnCate.off('click.clickBtnCate').on('click.clickBtnCate',function (e){
            e.preventDefault();
            const $self = $(this);
            paging = {pageIdx: 0, pageEnd: 0, increase: 0, list: []};
