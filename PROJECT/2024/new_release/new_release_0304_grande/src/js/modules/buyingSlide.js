...
export class BuyingSlide {
            ...
            on: {
                init: function () {
                    ...
                    if(buyingSlide.options.autoplay){
                        const $btnPlay = $target.find('.pt_btn-play');
                        $btnPlay.off(`click.clickAutoplay${target}`).on(`click.clickAutoplay${target}`, function() {
                            $btnPlay.toggleClass("pt_pause pt_play");
                            const isPaused = $(this).hasClass('pt_pause');
                            if(isPaused) {
    ...
    update(slideId, isOnce, useTab) {
        ...
        function mainSlide(){
            const $target = $(buyingSlide.options.wrapper).find(buyingSlide.options.target);
