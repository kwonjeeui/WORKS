...
export const sns = {
    init(){
        ...
        PT_STATE.$PROJECT.on('click', '[data-role="btnSns"]', function (e) {
            ...
            if (type === 'fb') { // 페이스북
                window.open(`https://www.facebook.com/sharer/sharer.php?u=https:${encodeURIComponent(url)}menubar=no,toolbar=no,resizable=yes,scrollbars=yes,width=800,height=650`);
            } else if (type === 'blog' || type ==='cafe') { // 네이버블로그
                window.open(`http://share.naver.com/web/shareView.nhn?url=https:${encodeURIComponent(url)}&title=${encodeURIComponent(title)}menubar=no,toolbar=no,resizable=yes,scrollbars=yes,width=800,height=650`);
            } else if (type === 'insta') {
