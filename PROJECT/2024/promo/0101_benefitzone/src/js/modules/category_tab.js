...
export const category_tab = {
    init(){
        ...
        var flag = false;
        var slideLength = -1;
        var $component = $('.wrap-component');
        var $category = $('.pt_category_box');
        var categorySwiper = [];//각 카테고리 스와이퍼
        var cateEnd = [];//각 카테고리의 마지막 컴포넌트
        var cateLength = [];//각 카테고리의 li갯수
        var cateAnc = [];//컴포넌트 앵커이동 제어 true:앵커이동/false:탭작동 초기값 true
        var cateSwp = [];//모바일 카테고리 스와이퍼 제어 true:작동on/false:작동off 초기값 true
        var compLength = 0;
        var compSumLength = 0;
        ...
        var scrollMove = function() {
            ...
            $category.each(function(index,el){
                ...
                if (_scrollTop >= $this.offset().top && _scrollTop < cateEnd[index].offset().top + cateEnd[index].outerHeight() && !flag) {
                    ...
                    $component.each(function(idx, item) {
                        ...
                        if (_scrollTop >= $item.offset().top && _scrollTop < $item.offset().top + $item.outerHeight()) {
                            $this.find('.category_list').each(function() {

                                var $target = $(this).find('li').eq(idx - compSumLength);
                                if (!$target.hasClass('on')) {
                                    $target.addClass('on').siblings().removeClass('on');
                                    $this.find('.category_list a').find('.selected_option').remove();
        ...
        PT_STATE.$PROJECT.off('click.clickCate').on('click.clickCate', '.category_list a', function(e) {
            e.preventDefault();
            var $this = $(this).closest('li');
            var click_idx = $this.index();
            var parenstName = $this.closest('.pt_category_box');
            compLength = 0;
            compSumLength = 0;
            $('.category_list a').find('.selected_option').remove();
            $(this).append('<span class="blind selected_option">선택됨</span>')                

            if (!flag) {
                flag = true;

                $this.closest('.pt_slider_category_wrap').find('.category_list').each(function() {
                ...
                $category.each(function(index,el){
                    ...
                    if (parenstName.hasClass('pt_cate'+ index)) {
                        ...
                        $('html, body').stop().animate({scrollTop:$component.eq(click_idx + compSumLength).offset().top - $this.find('.pt_slider_category_wrap').outerHeight() + 1}, 500, function() {flag = false;});
                    }
                    compSumLength += $this.find('li').length;
